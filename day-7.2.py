...
def hand_type(hand: list) -> int:
    ...
    for index, card in enumerate(input_hand):
        if card == "A":
            ...
            card_freq[12] += 1
            output_hand[index + 1] = 1
        elif card == "T":
            card_freq[3] += 1
            output_hand[index + 1] = 10
        else:
            card_value = int(card)
            card_freq[13 - card_value] += 1
            output_hand[index + 1] = card_value

    # Account for jokers
    jokerfied_freq = card_freq[:12]
    num_jokers = card_freq[12]
    jokerfied_freq[np.argmax(jokerfied_freq)] += num_jokers

    # Determine the type of the hand
    if max(jokerfied_freq) == 5:
        output_hand[0] = 7
    elif max(jokerfied_freq) == 4:
        output_hand[0] = 6
    elif max(jokerfied_freq) == 3 and np.count_nonzero(jokerfied_freq) == 2:
        output_hand[0] = 5
    elif max(jokerfied_freq) == 3:
        output_hand[0] = 4
    elif max(jokerfied_freq) == 2 and np.count_nonzero(jokerfied_freq) == 3:
        output_hand[0] = 3
    elif max(jokerfied_freq) == 2:
        output_hand[0] = 2
    else:
        output_hand[0] = 1
